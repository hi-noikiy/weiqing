...
function app_navs($type = 'home', $multiid = 0, $section = 0,$styleid) {
	global $_W;
	$pos = array();
	$pos['home'] = 1;
	$pos['profile'] = 2;
	...
	$sql = "SELECT id,name, description, url, icon, css, position, module,styleid FROM " . tablename('site_nav') . " WHERE position = '{$pos[$type]}' AND status = 1 AND uniacid = '{$_W['uniacid']}' AND multiid = '{$multiid}' AND styleid='{$styleid}' ORDER BY displayorder DESC, id ASC";
