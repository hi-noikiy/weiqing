...
function site_navs($params = array()) {
	...
	if(!$cid || !$ishomepage) {
		...
		$navs = pdo_fetchall("SELECT id, name, description, url, icon, css, position, module,styleid FROM ".tablename('site_nav')." WHERE position = '1' AND status = 1 AND uniacid = '{$_W['uniacid']}' AND styleid='{$_GPC['s']}' AND multiid = '{$multiid}' $condition ORDER BY displayorder DESC, id DESC");
	} else {
