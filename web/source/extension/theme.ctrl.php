...
if ($do == 'designer') {
	...
	if (checksubmit('submit') && $available[$_GPC['method']]) {
        ...
        $t['template']['url_index'] = trim($_GPC['template']['url_index']);
        if(empty($t['template']['url_index']) || preg_match('/\*\/|\/\*|eval|\$\_/i', $t['template']['url_index'])) {
            message('请输入有效的模板发布页');
        }
...
<manifest versionCode="{$versions}">
	...
	<url_index><![CDATA[{$t['template']['url_index']}]]></url_index>
