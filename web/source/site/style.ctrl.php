...
if ($do == 'template') {
	...
	$styles = pdo_fetchall("SELECT a.*,b.url_index FROM ".tablename('site_styles')." AS a LEFT JOIN ".tablename('site_templates')." AS b ON a.templateid = b.id WHERE uniacid = :uniacid ".(!empty($where) ? " AND $where" : ''), $params);
	...
	foreach($styles as $v) {
		...
		$stylesResult[] =  array(
            ...
            'url_index' => $templates[$v['templateid']]['url_index']
		);
	}
