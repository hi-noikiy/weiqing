...
if ($do == 'design') {
	...
	if (!empty($_GPC['wapeditor'])) {
		...
		message('页面保存成功.', url('site/uceditor/design', array('id' => $id, 'multiid' => $multiid)), 'success');
	} else {
	...
	if (!empty($_GPC['wapeditor'])) {
		...
		message('个人中心保存成功.', url('site/uceditor/uc'), 'success');
	}
	...
	if(!empty($_GPC['styleid'])){
		$site_navs = pdo_fetchall("SELECT * FROM ".tablename('site_nav')." WHERE uniacid = :uniacid AND position = '1' AND status = 1 AND styleid=:styleid",array(':uniacid' => $_W['uniacid'],':styleid'=>$_GPC['styleid']));
	}
	template('site/uceditor');
