...
class Ewei_DShop_Member
{
	...
	public function getCredits($openid = '', $type = array('credit1', 'credit2'))
	{
		...
		if (!empty($uid)) {
			...
			return pdo_fetch("SELECT {$credittype} FROM " . tablename('ewei_shop_member') . ' WHERE  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
