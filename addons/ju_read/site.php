...
class ju_readModuleSite extends WeModuleSite {
	...
	public function doWebAjax() {
		...
		if($operation=='deleteprizes'){
			$id=intval($_GPC['id']);
			if(empty($id))die(json_encode(array('success'=>false,"msg"=>"参数不能为空")));
			pdo_delete('ju_read_prizes',array('id'=>$id));
	...
	public function doMobileMain() {
		...
		if (!empty($reply)) {
			if ($reply['starttime'] > time()) {//检测时间是否开始
				...
				if (empty($_W['openid']))message("请在微信中访问");
				...
				$list_prizes = pdo_fetchall("SELECT * FROM ".tablename("ju_read_prizes")." WHERE rid = :rid ORDER BY `displayorder` desc,`id` DESC", array(':rid' => $reply['rid']));
				$ary_prizes=array();
				foreach($list_prizes as $row){
					$ary_prizes[$row['id']]=array(
						'id'=>$row['id'],
	...
	public function doMobileDuijiang() {
		...
		$prizes =  pdo_fetch("SELECT * FROM ".tablename("ju_read_prizes")." WHERE id = :id  ", array(':id'=>$itemId));
		if (empty($_W['openid']) || empty($reply_id)) {
			...
			if (empty($log)) {
			...
			}elseif ($log['hits'] < $prizes['neednum']) {
				$data['status'] = '3';//阅读数不够
				$data['msg'] = '抱歉，您的阅读数不够，不能领取该奖励';
			}elseif ($prizes['prizesy'] < 1) {
				...
				pdo_update('ju_read_data',$update,array('id'=>$log['id']));
				pdo_update('ju_read_prizes',array('prizesy'=>$prizes['prizesy']-1),array('id'=>$prizes['id']));
				$data['status'] = '1';
